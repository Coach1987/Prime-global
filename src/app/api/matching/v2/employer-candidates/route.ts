...
export async function GET(request: Request) {
  ...
  const [{ data: candidates, error: candidatesError }] = await Promise.all([
    supabase.from("candidate_profiles").select("id, full_name, professional_title, country, city, settings").order("updated_at", { ascending: false }).limit(300),
  ]);
  ...
  const insights = (candidates ?? []).map((candidate) => ({ candidateId: candidate.id, candidateName: candidate.full_name ?? "Candidate", ...buildEmployerMatchInsight(employer, candidate) })).sort((left, right) => right.compatibilityScore - left.compatibilityScore);
